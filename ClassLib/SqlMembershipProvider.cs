...
You will have to change SqlMembershipProvider::encryptionKey to a random hexadecimal value of your choice
    ...
    public sealed class SqlMembershipProvider : MembershipProvider
    {
                 ...
                 out MembershipCreateStatus status)
        {
            ...
            if (u == null)
            {
                ...
                cmd.Parameters.Add("@PKID", SqlDbType.UniqueIdentifier).Value = Guid.NewGuid(); // new Guid(providerUserKey.ToString());
                cmd.Parameters.Add("@Username", SqlDbType.VarChar, 255).Value = username;
                ...
                cmd.Parameters.Add("@PasswordAnswer", SqlDbType.VarChar, 255).Value = passwordAnswer == null ? null : EncodePassword(passwordAnswer);
                ...
                cmd.Parameters.Add("@ApplicationName", SqlDbType.VarChar, 255).Value = pApplicationName;
                cmd.Parameters.Add("@IsLockedOut", SqlDbType.Bit).Value = 0;	//false
        ...
        {
            SqlConnection conn = new SqlConnection(connectionString);
            string sqlString = "SELECT Count(*) FROM " + tableName + " WHERE ApplicationName = @ApplicationName";
        ...
        public override string GetPassword(string username, string answer)
        {
            ...
            SqlConnection conn = new SqlConnection(connectionString);
            SqlCommand cmd = new SqlCommand("SELECT Password, PasswordAnswer, IsLockedOut FROM " + tableName + 
                  " WHERE Username = @Username AND ApplicationName = @ApplicationName", conn);
                ...
                using (reader = cmd.ExecuteReader())
                {
                    if (reader.HasRows)
                    {
                        ...
                        if (userIsOnline)
                        {
                            ...
                            updateCmd.Parameters.Add("@LastActivityDate", SqlDbType.VarChar).Value = DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss");
        ...
        public override MembershipUser GetUser(object providerUserKey, bool userIsOnline)
        {
            ...
            cmd.Parameters.Add("@PKID", SqlDbType.UniqueIdentifier).Value = new Guid(providerUserKey.ToString());
                ...
                using (reader = cmd.ExecuteReader())
                {
                    if (reader.HasRows)
                    {
                        ...
                        if (userIsOnline)
                        {
                            ...
                            updateCmd.Parameters.Add("@LastActivityDate", SqlDbType.VarChar).Value = DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss");
                            updateCmd.Parameters.Add("@PKID", SqlDbType.UniqueIdentifier).Value = new Guid(providerUserKey.ToString());
        ...
        public override void UpdateUser(MembershipUser user)
        {
            ...
            cmd.Parameters.Add("@Email", SqlDbType.VarChar, 128).Value = user.Email == null ? string.Empty : user.Email;
            cmd.Parameters.Add("@Comment", SqlDbType.VarChar, 255).Value = user.Comment == null ? string.Empty : user.Comment;
            cmd.Parameters.Add("@IsApproved", SqlDbType.Bit).Value = user.IsApproved;
            cmd.Parameters.Add("@Username", SqlDbType.VarChar, 255).Value = user.UserName == null ? string.Empty : user.UserName;
            cmd.Parameters.Add("@ApplicationName", SqlDbType.VarChar, 255).Value = pApplicationName == null ? string.Empty : pApplicationName;
        ...
        public override bool ValidateUser(string username, string password)
        {
            ...
            SqlConnection conn = new SqlConnection(connectionString);
            SqlCommand cmd = new SqlCommand("SELECT Password, IsApproved FROM " + tableName + 
                    " WHERE Username = @Username AND ApplicationName = @ApplicationName AND IsLockedOut = 0", conn);
            ...
            try
            {
                ...
                if (CheckPassword(password, pwd))
                {
                    if (isApproved)
                    {
                        ...
                        SqlCommand updateCmd = new SqlCommand("UPDATE " + tableName + " SET LastLoginDate = @LastLoginDate, LastActivityDate = @LastActivityDate" +
                                                                " WHERE Username = @Username AND ApplicationName = @ApplicationName", conn);

                        updateCmd.Parameters.Add("@LastLoginDate", SqlDbType.DateTime).Value = DateTime.Now;
                        updateCmd.Parameters.Add("@LastActivityDate", SqlDbType.DateTime).Value = DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss");
                        updateCmd.Parameters.Add("@Username", SqlDbType.VarChar, 255).Value = username;
                        updateCmd.Parameters.Add("@ApplicationName", SqlDbType.VarChar, 255).Value = pApplicationName;
        ...
        private string UnEncodePassword(string encodedPassword)
        {
            ...
            switch (PasswordFormat)
            {
                ...
                case MembershipPasswordFormat.Encrypted:
                    ...
                    {
                        throw new ProviderException("DecryptPassword(Convert.FromBase64String(password)) Failed:" + e);
                    }
                    break;
                case MembershipPasswordFormat.Hashed:
