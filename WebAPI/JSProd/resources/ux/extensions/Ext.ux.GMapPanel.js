...
markers: [{
     ...
     * @param polygons, googleMap
     */
    addPolyLines : function(polygons, gMap) {
        var polyLine, polyLineObj = [];
        if (!gMap) {
             gMap = this.getMap();
        }
        if (Ext.isArray(polygons)){
            for (var i = 0; i < polygons.length; i++) {
                if (typeof polygons[i] === "object") {
                    polyLine = new GPolyline([
                        new GLatLng(polygons[i].Lat1, polygons[i].Lng1),
                        new GLatLng(polygons[i].Lat2, polygons[i].Lng2)
                    ], polygons[i].LineColor, polygons[i].LineWeight, polygons[i].LineOpacity);
                    polyLineObj[polyLineObj.length] = polyLine;
     ...
     * @param polygons
     */
    removePolyLines : function(polygons, gMap) {
        var polyLine, polyLineObj = [];
        if (!gMap) {
             gMap = this.getMap();
        }
